...
class ResearchServerService {
  // -------------------------------------------------------------------------
  // TODO: Replace this with the actual research server URL once the server is
  // built. While this placeholder is present, the sync methods will skip the
  // network call and surveys will remain in the local database for later
  // upload.
  // -------------------------------------------------------------------------
  static const String _researchServerUrl =
      'https://research-server.example.com/api/v1/surveys';

  static bool get _isServerConfigured =>
      !_researchServerUrl.contains('example.com');

  // RSA-2048 public key used for key encapsulation.
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        final response = await http
            .post(
              Uri.parse(_researchServerUrl),
              headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'User-Agent': 'BarcelonaWellbeingMapper/1.0',
              },
              body: jsonEncode({
