...
class _InteractiveLocationPrivacyMapState extends State<InteractiveLocationPrivacyMap> {
  ...
  void _processEraserAction(Offset globalPosition) {
    ...
    try {
      tapLatLng = camera.offsetToCrs(localPosition);
    } catch (e) {
      print('[InteractiveMap] ⚠️ offsetToCrs failed ($e), falling back to bounds interpolation');
    }

    if (tapLatLng == null) {
      final bounds = camera.visibleBounds;
      final relativeX = (localPosition.dx / mapSize.width).clamp(0.0, 1.0);
      final relativeY = (localPosition.dy / mapSize.height).clamp(0.0, 1.0);
      final tapLat = bounds.north - (relativeY * (bounds.north - bounds.south));
      final tapLng = bounds.west + (relativeX * (bounds.east - bounds.west));
      tapLatLng = LatLng(tapLat, tapLng);
    }

    _currentEraserLatLng = tapLatLng;

    print('[InteractiveMap] Touch at map-local: ${localPosition.dx.toStringAsFixed(1)}, ${localPosition.dy.toStringAsFixed(1)} -> lat/lng: ${tapLatLng.latitude.toStringAsFixed(6)}, ${tapLatLng.longitude.toStringAsFixed(6)}');
    print('[InteractiveMap] Map size: ${mapSize.width.toStringAsFixed(1)} x ${mapSize.height.toStringAsFixed(1)}');
    
    _findAndToggleNearbyPoints(tapLatLng);
  }
