...
class AppModeService {
  ...
  static Future<bool> sendsDataToResearch() async {
    final mode = await getCurrentMode();
    if (isDemoBuild) {
      debugPrint('[AppModeService] Demo build detected - forcing sendsDataToResearch=false');
      return false;
    }
  ...
  /// Clear stored mode data - useful for testing/debugging
  static Future<void> clearModeData() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove(_modeKey);
    await prefs.remove('testing_participant_code');
  }
  ...
  static Future<void> logModeStatus() async {
    ...
    try {
      ...
      debugPrint('Is Demo Build: $isDemoBuild');
